...
function search(ei,ef,tsearch,data){
 ...
 // console.log("Coordenadas",network.getPositions(1));
  ...
  console.log("inicial",ei,"node end",ef);
  console.log("data ne search",data);
  console.log("_________________________");
...
function asterisco(ei,ef)
{
 ...
 // var matrixEDP=new Array(3);
  ...
  for(var i=0;i<meta.length;i++){
    ...
    data.nodes._data[meta[i]].group = 'ruta';
  }
  txt+="}"
...
function trazaRutaSolucionRed(meta){

    
  for(var i=0;i<meta.length;i++){
    if(i!=meta.length-1){
    var posi=findToFrom(meta[i],meta[i+1]);
    console.log("posicion",  posi);
   
        data.edges._data[posi].colore = "red";
    //}
    //else{
      //data.edges._data[posi].colore = "red";
    //}
...
function findToFrom(to1,from1)
{
  console.log("tamnanio",data.edges.length);
  for(var j=1; j<=edges.length ; j++){
        if(data.edges._data[j].to==to1 && data.edges._data[j].from==from1 || data.edges._data[j].to==from1 && data.edges._data[j].from==to1){
            return data.edges._data[j].id;;
            console.log("buu alma",id);
        }

  }
}
function reseteaAtriNode(meta){
  for(var i=0;i<meta.length;i++){
    data.nodes._data[meta[i]].group = null;
  }
}
